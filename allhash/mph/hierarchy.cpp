...
: hash(0ul), base(nullptr), subtype(1) {}
...
const klass_t* hierarchy_t::add(std::vector<std::size_t> const& vec)
{
  ...
  { // first path item : current pole
    std::size_t hash = boost::lexical_cast<std::size_t>(vec.front());
...
template void hierarchy_t::order<std::vector<klass_t const*>>(std::vector<klass_t const*>&);
